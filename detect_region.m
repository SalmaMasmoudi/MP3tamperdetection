function [bg_region, tab_len_frame, index_bg_region,nbframe,index_iter1,x1,indice1]= detect_region (file_name,D1,D2,D3,D4)
    clc;
 ...
 x1=x;
    %File Read Bit By Bit and converted to uint8 format for conserving memory
    ...
    huffman_data_bits(1,:) = double(0);
   
  bg_region1(1,:)=double(0);
index_bg_region1(1,:)=double(0);
index_iter(1,:)=double(0);
bg_region(1,:)=double(0);
index_bg_region(1,:)=double(0);
index_iter1(1,:)=double(0);
tab_main(1:4,:)= double(0);
indice1(1:4,:)=double(0);
...
%                                             if(D1(fr,huf_iter)==0 && D1(fr,huf_iter+1)==0)||(D2(fr,huf_iter)==0 && D2(fr,huf_iter+1)==0)||(D3(fr,huf_iter)==0 && D3(fr,huf_iter+1)==0)||(D4(fr,huf_iter)==0 && D4(fr,huf_iter+1)==0)
                                             bg_region1(1,ran)=x(i+max_cl0-1);
                                          
                                             
                                             index_bg_region1(ri,ran)=i+max_cl0-1;
                                            
                                             index_iter(1,dan:dan+1)=[huf_iter huf_iter+1];
                                             dan=dan+2;
                                             ran=ran+1;
%                                             end
                                    ...
                                    elseif huf_iter > add1_range(1,2) && huf_iter <= add2_range(1,2)
                                        if table_select(ri,2) ~= 0
                                         ...
                                         %  if(D1(fr,huf_iter)==0 && D1(fr,huf_iter+1)==0)||(D2(fr,huf_iter)==0 && D2(fr,huf_iter+1)==0)||(D3(fr,huf_iter)==0 && D3(fr,huf_iter+1)==0)||(D4(fr,huf_iter)==0 && D4(fr,huf_iter+1)==0)
                                             bg_region1(1,ran)=x(i+max_cl1-1);
                                           
                                             index_bg_region1(ri,ran)=i+max_cl1-1;
                                             ...
                                             ran=ran+1;
                                         %  end
%                                              bg_region1(1,ran)=x(i+length_code-1);
%                                             
%                                              index_bg_region1(1,ran)=i+length_code-1;
%                                             
%                                            index_iter(1,dan:dan+1)=[huf_iter huf_iter+1];
%                                              dan=dan+2;
%                                              
                                    ...
                                    elseif huf_iter > add2_range(1,2) && huf_iter <= bg_val
                                        if table_select(ri,3) ~= 0
                                            ...
                                            linbits = linbits_region2;
                                             ...
                                             % if(D1(fr,huf_iter)==0 && D1(fr,huf_iter+1)==0)||(D2(fr,huf_iter)==0 && D2(fr,huf_iter+1)==0)||(D3(fr,huf_iter)==0 && D3(fr,huf_iter+1)==0)||(D4(fr,huf_iter)==0 && D4(fr,huf_iter+1)==0)                                        
                                          bg_region1(1,ran)=x(i+max_cl2-1);
                                          
                                             
                                             index_bg_region1(ri,ran)=i+max_cl2-1;
...
%                                              ran=ran+1;
                                           ...
                                           % huffman_data_bits(ri,add2_range(1,2)+1:bg_val-2)=0;
                                           % index_huffman_data(ri,add2_range(1,2)+1:bg_val-2)=add2_range(1,2)+1:bg_val-2;
...
taille(fr)=length(bg_region1);


index_bg_region=index_bg_region1;
bg_region=bg_region1;
...
%------------------------------------------------------------------------------------------------------------%
   ...
   nbframe=frame_counter;
%------------------------------------------------------------------------------------------------------------%
%%%%%%%%%%%%%%%%%%%%%------------------fermer et enregistre le fichier tatoué------------------%%%%%%%%%%%%%%%
%------------------------------------------------------------------------------------------------------------%               

tab_len_frame(1)=taille(1);

for dddd=2:length(taille)
 tab_len_frame(dddd)=taille(dddd)-taille(dddd-1);
end
% j=tab_len_frame(1);
%  for nn=1:nbframe-1
% %     S.(nn)=index_bg_region(nn,tab_len_frame(nn))
% indice1(:,nn)=index_bg_region(:,j+1);
% j=j+tab_len_frame(nn)-1;
%  end
compt=1;
for nn=1:4
    for j=1:length(index_bg_region)
        if (index_bg_region(nn,j))>0
           % pause
        indice1(compt)=index_bg_region(nn,j);
        compt=compt+1;
        end
    end
end
fclose(fid);
disp(sprintf('Time Taken To Decode Frame: %f Seconds',total_time ));
fprintf('Time Taken To Decode Frame: %f Seconds\n',toc);
  
save  data.mat index_bg_region bg_region indice1 x
